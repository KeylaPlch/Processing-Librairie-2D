 ...
 */
static class DetectionCollision {
  /**
   * Sketch global.
   */
  ...
  /**
   * Initialisation de la détection de collision.
   * @param _sketch Sketch global.
   */
  public static void initialisation(PApplet _sketch) {
    sketch = _sketch;
  }

  /**
   * Vérifier si il y a une collision entre un acteur et une limite.
   * @param _limite Limite à vérifier.
   * @param _acteur Acteur à vérifier.
   */
  static void collision(Limite _limite, Acteur _acteur) {
    /* Pas de collision si l'acteur a été supprimé. */
    if (_acteur.supprimer) return;
    /* Pas de collision si l'acteur n'a pas bougé depuis la dernière trame. */
    if (_acteur.x == _acteur.precedent.x && _acteur.y == _acteur.precedent.y) return;

    /* Récupération de la correction de la position de l'acteur. */
    float[] correction = bloquer(_limite, _acteur);
    if(correction != null) {
      _limite.notifierAuditeur(_acteur, correction);
      _acteur.attacherLimite(_limite, correction);
    }
  }

  /**
   * Vérifier si un acteur doit être bloqué par une limite.
   * @param _limite Limite à vérifier.
   * @param _acteur Acteur à vérifier.
   * @return Correction de la position de l'acteur.
   */
  static float[] bloquer(Limite _limite, Acteur _acteur) {
    float[] ligne = {_limite.x, _limite.y, _limite.x2, _limite.y2};
    float[] precedent =_acteur.precedent.recupererDelimitations();
    float[] actuel = _acteur.recupererDelimitations();
    return DetectionCollision.getLineRectIntersection(ligne, precedent, actuel);
  }
   ...
   */
  static float[] translationRotation(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, float angle, float cosine, float sine)
  {
